<template>
	...
	<Demo :html="html" :js="js">
		<i-checkbox
			...
			v-model="model"
			:options="options"
			:type="config.type as CheckboxType"
			:option-inline="config.optionInline"
		...
		<template #config>
			...
			>
				...
				<i-checkbox v-model="config.optionInline" type="switch">
					option-inline
				</i-checkbox>
	...
	<h5 id="option" class="mb-12 mt-40 green">Option</h5>
...
const propsGuidence = [
	{
		...
		type: "boolean | Array",
		defaultValue: "",
		desc: "表单数据模型，只有一个选项的时候为 boolean 类型，有 options 时的时候会自动转为数组类型",
	},
	{
		name: "options",
		type: "Option[]",
