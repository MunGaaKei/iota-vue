...
<template>
    ...
    <Teleport v-if="body" to="body">
        <div ref="$popup" v-if="active" class="i-popup" :style="popupStyle">
            <slot></slot>
        </div>
    </Teleport>

    <div v-if="!body && active" ref="$popup" class="i-popup" :style="popupStyle">
        <slot></slot>
    </div>
</template>

<script setup lang="ts" name="i-popup">
import { useSlots, h, withDefaults, VNode, withModifiers, computed, ref, nextTick, StyleValue } from "vue";
...
function computePopupPos (): void {
    ...
    if ( props.position ) {
        [posLeft, posTop] = computePosWithDirection(props.position, rectPopup, rectTrigger, $trigger.value);
    } else {
        let preferPosition: TypePosition = (rectTrigger.top - rectPopup.height - props.gap * 2 >= 0)? 'top': 'bottom';
        [posLeft, posTop] = computePosWithDirection(preferPosition, rectPopup, rectTrigger, $trigger.value);
    }
    
    offsets.value = `translate3d(${posLeft}px, ${posTop}px, 0)`;
}

function computePosWithDirection (direction: TypePosition, rectPopup: DOMRect, rectTrigger: DOMRect, $trigger: HTMLElement): [number, number] {
    let w = 0;
    let h = 0;
    let { left: tl, top: tt, right: tr, bottom: tb, width: tw, height: th } = rectTrigger;
...
function computeOffset ({
    ...
    popupSize: number,
    gap: number
}, alignCenter: boolean = true): number {
...
function getNativeEventType <TypeTrigger extends string> (event: TypeTrigger): string[] {
    switch (event) {
        case 'hover':
